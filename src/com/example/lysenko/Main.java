...
abstract class Cases implements Initiate {
	...
	protected int search(ArrayList<Query> listQuery, Query query, ArrayList<WaitingTimeline> listWaitingTimeline,
			LinkedList<String> linkedList, int countEqual) {
		...
		for (int j = 0; j < listWaitingTimeline.size(); j++) {
			if (query.service_id == listWaitingTimeline.get(j).service_id
					...
					|| query.question_type_id == SYMBOL_ALL
							&& listWaitingTimeline.get(j).response_type.equals(query.response_type)
							&& listWaitingTimeline.get(j).date_from.getTime().after(query.date_from.getTime())
					|| query.question_type_id == SYMBOL_ALL
							&& listWaitingTimeline.get(j).date_from.getTime().before(query.date_to.getTime())
					|| query.question_type_id == SYMBOL_ALL
							&& listWaitingTimeline.get(j).date_from.getTime().equals(query.date_from.getTime())
					|| query.question_type_id == SYMBOL_ALL
							&& listWaitingTimeline.get(j).date_from.getTime().equals(query.date_to.getTime())) {
				countEqual++;
	...
	public void enterData() {
		...
		for (int indexLeft = 0; indexLeft < indexRight - 1; indexLeft++) {
			...
			if (inputWords[0].equals("C") && arrayList.size() <= countLines) {
				...
				linkedList.add(
						String.valueOf(query.search(listQuery, query, listWaitingTimeline, linkedList, countEqual)));
			}
		}
